...
import { registrasiKaryawan } from '@/services/KaryawanApi';

function RegistrasiKaryawan() {
  const [nama, setNama] = useState<string>('');
  const [email, setEmail] = useState<string>('');
  const [password, setPassword] = useState<string>('');
  const [peran, setPeran] = useState<string>('karyawan');
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const respon = await registrasiKaryawan(nama, email, password, peran);
      ...
      <Container maxWidth="xs">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <ToggleButtonGroup
            ...
            sx={{ mt: 2 }}
          >
            <ToggleButton value="karyawan">Karyawan</ToggleButton>
